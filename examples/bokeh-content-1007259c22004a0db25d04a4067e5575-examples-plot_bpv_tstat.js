(function() {
  const fn = function() {
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
    
    const element = document.getElementById("7a91a24f-e8c6-4b55-9145-bf4990f91112");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '7a91a24f-e8c6-4b55-9145-bf4990f91112' but no matching script tag was found.")
        }
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.3.min.js"];
      const css_urls = [];
    
      const inline_js = [    function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                  const docs_json = '{"e202e21c-0583-45a2-aeb5-53f827992f38":{"defs":[],"roots":{"references":[{"attributes":{"children":[{"id":"18076"},{"id":"18074"}]},"id":"18077","type":"Column"},{"attributes":{"overlay":{"id":"18012"}},"id":"18007","type":"LassoSelectTool"},{"attributes":{},"id":"17989","type":"DataRange1d"},{"attributes":{},"id":"18037","type":"BasicTickFormatter"},{"attributes":{"axis":{"id":"17995"},"coordinates":null,"group":null,"ticker":null},"id":"17998","type":"Grid"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18053","type":"Circle"},{"attributes":{},"id":"17996","type":"BasicTicker"},{"attributes":{},"id":"18044","type":"UnionRenderers"},{"attributes":{"coordinates":null,"formatter":{"id":"18040"},"group":null,"major_label_policy":{"id":"18041"},"ticker":{"id":"17996"}},"id":"17995","type":"LinearAxis"},{"attributes":{},"id":"18040","type":"BasicTickFormatter"},{"attributes":{},"id":"17991","type":"LinearScale"},{"attributes":{},"id":"18003","type":"ResetTool"},{"attributes":{"coordinates":null,"data_source":{"id":"18028"},"glyph":{"id":"18029"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18031"},"nonselection_glyph":{"id":"18030"},"view":{"id":"18033"}},"id":"18032","type":"GlyphRenderer"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"18011","type":"BoxAnnotation"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18025","type":"Line"},{"attributes":{"source":{"id":"18028"}},"id":"18033","type":"CDSView"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18024","type":"Line"},{"attributes":{"data":{},"selected":{"id":"18045"},"selection_policy":{"id":"18044"}},"id":"18028","type":"ColumnDataSource"},{"attributes":{},"id":"18038","type":"AllLabels"},{"attributes":{"coordinates":null,"formatter":{"id":"18037"},"group":null,"major_label_policy":{"id":"18038"},"ticker":{"id":"18000"}},"id":"17999","type":"LinearAxis"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"18043"},"selection_policy":{"id":"18042"}},"id":"18022","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"18056","type":"Title"},{"attributes":{"tools":[{"id":"18003"},{"id":"18004"},{"id":"18005"},{"id":"18006"},{"id":"18007"},{"id":"18008"},{"id":"18009"},{"id":"18010"}]},"id":"18013","type":"Toolbar"},{"attributes":{"overlay":{"id":"18011"}},"id":"18005","type":"BoxZoomTool"},{"attributes":{},"id":"18008","type":"UndoTool"},{"attributes":{"coordinates":null,"data_source":{"id":"18050"},"glyph":{"id":"18051"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18053"},"nonselection_glyph":{"id":"18052"},"view":{"id":"18055"}},"id":"18054","type":"GlyphRenderer"},{"attributes":{"callback":null},"id":"18010","type":"HoverTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18029","type":"Line"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18051","type":"Circle"},{"attributes":{},"id":"18043","type":"Selection"},{"attributes":{},"id":"18069","type":"UnionRenderers"},{"attributes":{},"id":"18042","type":"UnionRenderers"},{"attributes":{"toolbars":[{"id":"18013"}],"tools":[{"id":"18003"},{"id":"18004"},{"id":"18005"},{"id":"18006"},{"id":"18007"},{"id":"18008"},{"id":"18009"},{"id":"18010"}]},"id":"18075","type":"ProxyToolbar"},{"attributes":{},"id":"18009","type":"SaveTool"},{"attributes":{},"id":"18004","type":"PanTool"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"18049"}]},"id":"18048","type":"Legend"},{"attributes":{},"id":"17987","type":"DataRange1d"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"18012","type":"PolyAnnotation"},{"attributes":{"toolbar":{"id":"18075"},"toolbar_location":"above"},"id":"18076","type":"ToolbarBox"},{"attributes":{},"id":"18041","type":"AllLabels"},{"attributes":{},"id":"18006","type":"WheelZoomTool"},{"attributes":{},"id":"17993","type":"LinearScale"},{"attributes":{"source":{"id":"18050"}},"id":"18055","type":"CDSView"},{"attributes":{"source":{"id":"18022"}},"id":"18027","type":"CDSView"},{"attributes":{"children":[[{"id":"17986"},0,0]]},"id":"18074","type":"GridBox"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18031","type":"Line"},{"attributes":{},"id":"18000","type":"BasicTicker"},{"attributes":{},"id":"18045","type":"Selection"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18030","type":"Line"},{"attributes":{},"id":"18070","type":"Selection"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"18032"}]},"id":"18049","type":"LegendItem"},{"attributes":{"axis":{"id":"17999"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"18002","type":"Grid"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18052","type":"Circle"},{"attributes":{"below":[{"id":"17995"}],"center":[{"id":"17998"},{"id":"18002"},{"id":"18048"}],"height":500,"left":[{"id":"17999"}],"output_backend":"webgl","renderers":[{"id":"18026"},{"id":"18032"},{"id":"18054"}],"title":{"id":"18056"},"toolbar":{"id":"18013"},"toolbar_location":null,"width":500,"x_range":{"id":"17987"},"x_scale":{"id":"17991"},"y_range":{"id":"17989"},"y_scale":{"id":"17993"}},"id":"17986","subtype":"Figure","type":"Plot"},{"attributes":{"coordinates":null,"data_source":{"id":"18022"},"glyph":{"id":"18023"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18025"},"nonselection_glyph":{"id":"18024"},"view":{"id":"18027"}},"id":"18026","type":"GlyphRenderer"},{"attributes":{"data":{},"selected":{"id":"18070"},"selection_policy":{"id":"18069"}},"id":"18050","type":"ColumnDataSource"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18023","type":"Line"}],"root_ids":["18077"]},"title":"Bokeh Application","version":"2.4.3"}}';
                  const render_items = [{"docid":"e202e21c-0583-45a2-aeb5-53f827992f38","root_ids":["18077"],"roots":{"18077":"7a91a24f-e8c6-4b55-9145-bf4990f91112"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
    function(Bokeh) {
        }
      ];
    
      function run_inline_js() {
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();